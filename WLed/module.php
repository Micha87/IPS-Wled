<?php
	class WLed extends IPSModule{
		...
		private function SetColorThird(int $value)
		{
			$msg = strval($value);
		...
		public function sendAPI(string $payload)
		{			
			$msg = strval($payload);			
			$this->sendMQTT($this->ReadPropertyString('Topic').'/api',"$msg");
		}
	...
	private function createVariablenProfiles()
    {
		...
		if (!IPS_VariableProfileExists('Wled.Preset')) {
